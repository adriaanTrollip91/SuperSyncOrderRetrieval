                                            ...
                                            <tbody>
                                                @foreach ($order['line_items'] as $item)
                                                    <tr>
                                                        ...
                                                        <td> {{ $order_currency }} {{ number_format($item['price'], 2) }}
                                                            <span>x</span> {{ $item['quantity'] }} </td>
                                                        <td>
                                                        ...
                                                        <td style="display: none;" class="fulfill-th">
                                                            @if ($item['fulfillable_quantity'] > 0)
                                                                @if ($item['fulfillment_service'] === 'manual' || $item['fulfillment_service'] === 'app-fulifllment-service')
                                                                    <a href="#"
                                                                        class="btn btn-primary fulfill_this_item"
                                                                        data-line_item_id="{{ $item['id'] }}"
                                                                        data-qty="{{ $item['quantity'] }}">Fulfill</a>
                                                                @else
                                            ...
                                            @if (!empty($order['shipping_lines']))
                                                ...
                                                @if (!empty($order['shipping_lines']) && count($order['shipping_lines']) > 0)
                                                    <tr>
                                                        ...
                                                        <td class="text-truncate text-left"></td>
                                                        <td class="text-truncate text-right"><span
                                                                style="float:right">{{ $order_currency . ' ' . number_format($total_shipping, 2) }}</span>
                                                        </td>
                                                    </tr>
                                                @endif
                                            @endif
                                            @if (!empty($order['tax_lines']))
                                                @foreach ($order['tax_lines'] as $tax)
                                                    <tr>
                                                        ...
                                                        <td class="text-truncate text-left">
                                                            {{ $tax['title'] . ' ( ' . (float) $tax['rate'] * 100 . '% )' }}
                                                        </td>
                                                        <td class="text-truncate text-right"><span
                                                                style="float:right">{{ $order_currency . ' ' . number_format($tax['price'], 2) }}</span>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                                @if (!empty($order['tax_lines']) && count($order['tax_lines']) > 1)
                                                    <tr>
                                                        <td colspan="2" class="text-truncate">Total Taxes @if ($order['taxes_included'] == true)
                                                                (Inclusive)
                                                            @endif
                                                        </td>
                                                        <td class="text-truncate text-left"></td>
                                                        <td class="text-truncate text-right"><span
                                                                style="float:right">{{ $order_currency . ' ' . number_format($order['total_tax'], 2) }}</span>
                                                        </td>
                                                    </tr>
                                                @endif
                                            @endif
